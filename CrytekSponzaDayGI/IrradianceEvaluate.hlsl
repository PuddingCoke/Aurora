Texture2D<float4> irradianceCoeff : register(t0);
...
{   
    ...
    + A1 * irradianceCoeff[uint2(3, 0)].rgb * ((-sqrt(3.0) * N.x) / (2.0 * SQRT_PI))
    + A2 * irradianceCoeff[uint2(4, 0)].rgb * ((sqrt(15.0) * N.y * N.x) / (2.0 * SQRT_PI))
    + A2 * irradianceCoeff[uint2(5, 0)].rgb * ((-sqrt(15.0) * N.y * N.z) / (2.0 * SQRT_PI))
    + A2 * irradianceCoeff[uint2(6, 0)].rgb * ((sqrt(5.0) * (3.0 * N.z * N.z - 1.0)) / (4.0 * SQRT_PI))
    + A2 * irradianceCoeff[uint2(7, 0)].rgb * ((-sqrt(15.0) * N.x * N.z) / (2.0 * SQRT_PI))
    + A2 * irradianceCoeff[uint2(8, 0)].rgb * ((sqrt(15.0) * (N.x * N.x - N.y * N.y)) / (4.0 * SQRT_PI)));
}
