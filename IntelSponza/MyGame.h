...
public:
	...
	MyGame() :
		...
		gBaseColor(new RenderTexture(Graphics::getWidth(), Graphics::getHeight(), FMT::RGBA8)),
		...
		gRoughnessMetallic(new RenderTexture(Graphics::getWidth(), Graphics::getHeight(), FMT::RGBA8)),
		...
		screenSpaceReflection(new Shader("SSR.hlsl", ShaderType::Pixel))
	{
...

		{
			...
			Renderer::getDevice()->CreateInputLayout(layout, ARRAYSIZE(layout), SHADERDATA(deferredVS), inputLayout.ReleaseAndGetAddressOf());
		}
