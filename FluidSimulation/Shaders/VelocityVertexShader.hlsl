...
VertexOutput main(in uint vertexID : SV_VertexID)
{
    ...
    output.vL = output.vUv - float2(velocityTexelSize.x, 0.0);
    output.vR = output.vUv + float2(velocityTexelSize.x, 0.0);
    output.vT = output.vUv + float2(0.0, velocityTexelSize.y);
    output.vB = output.vUv - float2(0.0, velocityTexelSize.y);
